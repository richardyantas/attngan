...
def build_super_images(real_imgs, captions, ixtoword,
    ...
    # nvis = 8
    nvis = 2 # changed by richard because attn_maps length is 2
    ...
    for i in range(num):
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
                ...
                #PIL_im = Image.fromarray(np.uint8(img[0]))
                #PIL_att = Image.fromarray(np.uint8(one_map[0]))
                #PIL_im = Image.fromarray(img[0]).astype(np.uint8())
                #PIL_att = Image.fromarray(one_map[0]).astype(np.uint8())
                merged = \
