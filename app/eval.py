...
from cachelib.file import FileSystemCache as SimpleCache
...
def generate(caption, wordtoix, ixtoword, text_encoder, netG, s3, copies=2):
    # load word vector
    ...
    full_path = "https://scopicbucket.s3.sa-east-1.amazonaws.com/output/%s"
    ...
    for j in range(batch_size):
        for k in range(len(fake_imgs)):
            ...
            stream.seek(0) # Change stream position to beginning of file
            if copies > 2:
            ...
            s3.Object( BUCKET_NAME, "output/"+blob_name).upload_fileobj(stream)
            stream.close()
...
def eval(caption):
    ...
    t0 = time.time()
    urls = generate(caption, wordtoix, ixtoword, text_encoder, netG, None)
    t1 = time.time()
...
if __name__ == "__main__":
    caption = "the bird has a red crown and a black eyering that is round"
